...
class GAE:
    ...
    def fit(self, G, features=None, encoder_type="gcn", labels=None, mask_label=None, alpha=1.0, beta=1.0):
        ...
        for epoch in range(300):
            ...
            # Unsupervised loss: adjacency reconstruction with improved stability
            # Apply L2 normalization to embeddings for better stability
            z_norm = torch.nn.functional.normalize(z, p=2, dim=1)
            
            # Compute inner product for edge prediction
            inner_product = torch.matmul(z_norm, z_norm.t())
            
            # Apply sigmoid with temperature scaling for better numerical stability
            temperature = 5.0  # Increase temperature to prevent extreme values
            logits = inner_product / temperature
            
            # Create adjacency matrix - make it symmetric and handle properly
            adj_true = torch.zeros(data.x.size(0), data.x.size(0), device=data.x.device)
            row, col = data.edge_index
            adj_true[row, col] = 1.0
            adj_true[col, row] = 1.0  # Ensure symmetry for undirected graphs
            
            # Use binary_cross_entropy_with_logits which is more numerically stable
            # This function applies sigmoid internally, so we don't need to apply it manually
            unsup_loss = torch.nn.functional.binary_cross_entropy_with_logits(logits, adj_true)
